...
public class fanWind : MonoBehaviour
{
    ...
    Vector3 fanRotation; //fan base rotation values
    bool groundCheck; //a boolean that checks if Plume is on the ground
    ...
    void OnTriggerStay(Collider Col)
    {
        ...
        if (Col.tag == "Player")
        {
            ...
            GameObject.Find("Plume").GetComponent<CharacterMovement>().glideLockoutTimer -= 0;
            ...
            if (fanRotation.x == 180 && fanRotation.y == 0 && fanRotation.z == 0 && groundCheck != true)
            { //down not working for some reason
                GameObject.Find("Plume").GetComponent<CharacterMovement>().ChController.transform.position += (-Vector3.up * windForce * Time.deltaTime);
            }
    ...
    void OnTriggerExit(Collider Col)
    {
        ...
        if (Col.tag == "Player")
        {
            GameObject.Find("Plume").GetComponent<CharacterMovement>().glideLockoutTimer -= .25f;
